...
def api_query_categoria(usuario_id: str = "iphone_user", nombre: str = None, mes: str = None):
    ...
    try:
        from datetime import datetime
        from modules.database import inicializar_firebase, obtener_presupuestos_v2
        inicializar_firebase()
        if not mes:
            mes = datetime.now().strftime("%Y-%m")
        if not nombre:
...
def api_query_prestamos(usuario_id: str = "iphone_user", solo_pendientes: bool = True):
    ...
    try:
        from modules.database import inicializar_firebase, listar_prestamos
        inicializar_firebase()
        prestamos = listar_prestamos(usuario_id, solo_pendientes=solo_pendientes)
...
def api_query_balance(usuario_id: str = "iphone_user", mes: str = None):
    ...
    try:
        from datetime import datetime
        from modules.database import inicializar_firebase, obtener_balance_v2
        inicializar_firebase()
        if not mes:
            mes = datetime.now().strftime("%Y-%m")
        balance, ingresos, gastos, _ = obtener_balance_v2(usuario_id, mes)
...
def api_admin_reset_and_init(usuario_id: str = Form("iphone_user")):
    ...
    try:
        ...
        cuentas_creadas = 0
        for c in cuentas_default:
            from modules.database import crear_cuenta
            crear_cuenta(usuario_id, c["nombre"], c["tipo"], c["balance"])
            cuentas_creadas += 1
        ...
        for cat_nombre, cat_budget in cats_kebo:
            from modules.database import crear_categoria
