...
def registrar_transaccion(usuario_id: str, tipo: str, monto: float, categoria: str, descripcion: str):
    ...
    db.collection("finanzas").add({
        "usuario_id": str(usuario_id),
        "tipo": tipo.lower(), # 'gasto' o 'ingreso'
        "monto": float(monto),
        "categoria": categoria.capitalize(),
    ...
    # Verificar si superó presupuesto en caso de gasto
    alerta_presupuesto = ""
    if tipo.lower() == "gasto":
        ...
        if presup_doc.exists:
            ...
            if total_cat > limite:
                alerta_presupuesto = f"\n⚠️ **¡ALERTA DE PRESUPUESTO!** Has superado el límite de ${limite:,.0f} en *{categoria}* (Gastado: ${total_cat:,.0f}). ¡Controla esos gastos!"
