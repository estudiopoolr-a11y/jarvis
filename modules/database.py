...
if not firebase_admin._apps:
    ...
    # 2. Si no hay variable de entorno válida, buscar en el entorno local
    if not cred:
        cred_path = None
        json_files = glob.glob("jarvis*.json") + glob.glob("*.json")
        for f in json_files:
...
def obtener_tareas_pendientes(usuario_id: str = "default"):
    ...
    # Modo personal: Trae todas las tareas no completadas sin importar el ID de origen
...
def obtener_balance_financiero(usuario_id: str = "default"):
    ...
    # Modo personal: Trae todas las finanzas registradas en la base de datos
    if usuario_id in ["default", "all", None]:
